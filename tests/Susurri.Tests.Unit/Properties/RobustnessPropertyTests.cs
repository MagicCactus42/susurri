...
public class RobustnessPropertyTests
{
    ...
    public void KademliaMessage_Rejects_All_Unknown_Type_Tags()
    {
        ...
        // 1 byte type + 16 byte messageId + 32 byte senderId + 1 byte pubkey-len + 0 byte pubkey
        // = 50 bytes (the deserializer's minimum-size guard).
        for (int t = 0; t < 256; t++)
        {
            var tag = (byte)t;
            ...
            header[0] = tag;
            // pubKeyLen byte at offset 49 stays 0 (no pubkey bytes follow).
